...
export default function SettingsModal() {
  ...
  function handleTemporaryApiKeyChange(e: React.ChangeEvent<HTMLInputElement>) {
          ...
          <div ref={modalRef} className="relative w-full max-w-2xl p-4">
            <div className="relative rounded-lg bg-white shadow">
              <div className="flex min-h-[200px] flex-col items-center justify-start space-y-2 rounded-t p-4 md:p-5">
                ...
                <div className="flex w-full grow flex-col items-center justify-center space-y-4 divide-y-2">
                  {apiKey && !isEditing ? (
                    <div className="flex">
                      <div className="">
                        <span className="font-bold">API key:</span>
                        <span className="text-gray-500"> {apiKey.slice(0, 4) + "****" + apiKey.slice(-4)}</span>
                      </div>
                      <button className="mx-2 text-blue-500" onClick={handleEdit}>
                        Edit
                      </button>
                    </div>
                  ) : (
                    <div className="full flex">
                      <div className="relative h-10 w-full min-w-[200px]">
                        <input
                          type="text"
                          value={temporaryApiKey || apiKey || ""}
                          placeholder=" "
                          className="text-blue-gray-700 disabled:bg-blue-gray-50 placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 border-blue-gray-200 peer h-full w-full rounded-[7px] border border-t-transparent bg-slate-100 bg-transparent px-3 py-2.5 font-sans text-sm font-normal outline outline-0 transition-all placeholder-shown:border focus:border-2 focus:border-gray-900 focus:border-t-transparent focus:outline-0 disabled:border-0"
                          onChange={handleTemporaryApiKeyChange}
                        />
                      ...
                      <button type="button" className="px-3 text-blue-500" onClick={handleSave}>
                        Save
                      </button>
                      <button type="button" className="px-3 text-gray-500" onClick={clearAPIKey}>
                        Clear
                      </button>
                    </div>
                  )}
